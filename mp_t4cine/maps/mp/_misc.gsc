...
MiscConnect()
{
	for (;;)
	{
		...
		setObjectiveText(game["attackers"], "^3Sass' Cinematic Mod ^7- Ported to WAW by ^3Forgive");
		setObjectiveText(game["defenders"], "^3Sass' Cinematic Mod ^7- Ported to WAW by ^3Forgive");
...
MiscSpawn()
{
	...
	for (;;)
	{
		...
		thread GivePlayerKillstreak(); // Command args: [radar_mp, airstrike_mp, dogs_mp]
		thread MsgAbout();
		thread MsgWelcome();
...
GivePlayerKillstreak()
{
	...
	setDvar("mvm_killstreak", "^8[radar_mp artillery_mp dogs_mp]");
	for (;;)
	{
		if(getDvar("mvm_killstreak") != "^8[radar_mp artillery_mp dogs_mp]")
		{
...
MsgAbout()
{
	...
	for (;;)
	{
		if(getdvar("about") != "About the mod...")
		{
			...
			wait 1.5;
			self IPrintLnBold("Ported to WAW by ^3Forgive");
			wait 1.5;
