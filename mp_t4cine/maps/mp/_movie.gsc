...
BotAim()
{
	...
	setDvar("mvm_bot_aim", "Bot aim at clostest enemy - ^8[name]");
	for (;;)
	{
		if(getDvar("mvm_bot_aim") != "Bot aim at clostest enemy - ^8[name]")
		{
			for ( i = 0; i < level.players.size; i++ )
			{
				player = level.players[i];
...
BotStare()
{
	...
	setDvar("mvm_bot_stare", "Bot stare at clostest enemy - ^8[name]");
	for (;;)
	{
		if(getDvar("mvm_bot_stare") != "Bot stare at clostest enemy - ^8[name]")
		{
			for ( i = 0; i < level.players.size; i++ )
			{
        	    player = level.players[i];
